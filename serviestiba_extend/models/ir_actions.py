# -*- coding: utf-8 -*-

from ast import literal_eval

from odoo import api, models


class IrActionsActWindow(models.Model):
    _inherit = "ir.actions.act_window"

    @api.model
    def _serviestiba_filter_confirmed_tender_lines(self):
        """Restrict the Softhealer Tender Lines action to confirmed lines only."""
        view = self.env.ref(
            "sh_all_in_one_tender_bundle.sh_bidline_tree_view",
            raise_if_not_found=False,
        )
        if not view:
            return True

        actions = self.browse()

        # Actions explicitly linked to this list view through ir.actions.act_window.view.
        action_views = self.env["ir.actions.act_window.view"].search([
            ("view_id", "=", view.id),
        ])
        actions |= action_views.mapped("act_window_id")

        # Some actions define their primary view directly.
        if "view_id" in self._fields:
            actions |= self.search([
                ("view_id", "=", view.id),
            ])

        actions = actions.filtered(lambda action: action.res_model == "purchase.order.line")

        for action in actions:
            domain = action.domain or "[]"
            if isinstance(domain, str):
                try:
                    domain = literal_eval(domain)
                except (ValueError, SyntaxError):
                    # Do not overwrite a dynamic/invalid domain blindly.
                    continue

            domain = list(domain or [])
            confirmed_filter = ("status", "=", "confirm")
            if confirmed_filter not in domain:
                domain.append(confirmed_filter)
                action.write({"domain": repr(domain)})

        return True
